...
# Run with no hang up: nohup time python encodeFileWritingUniformFiltered.py > efuout3-147_147.log&
...
def parks():
    ...
    # calculate polygon area and save in an array
    # poly_area=wkthelper.polyAreaArray(wktList, len(wktAll), len(wktAll))
    ...
    folder_name = "../encoding/uni_filtered_pk-"+str(minArea)+"-"+str(maxArea)+"-"+str(row_count)+"-"+str(col_count)
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
    filename=folder_name+"/weightint"
